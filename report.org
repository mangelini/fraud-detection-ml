...
We first split the dataset in two sets called /train \under set/ and /test \under set/ with a percentage of 90 and 10 respectively because Machine Learning and especially Deep Learning models needs a lot of data to execute the training. We put aside the /test \under set/ and we don't touch it anymore until the end.
...
It looks like most of the transactions are either CASH \under OUT or PAYMENT, so the majority of frauds will be of one of this two categories.
****  isFraud
